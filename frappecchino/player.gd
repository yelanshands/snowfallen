...
func _process(delta):
	camera.position = lerp(camera.position, spring_pos.position, delta*lerp_value)
...
func get_input():
	...
	crosshair.position.x = DisplayServer.window_get_size()[0]/2-(crosshair.size.x/2)
	crosshair.position.y = DisplayServer.window_get_size()[1]/2-(crosshair.size.y/2)
