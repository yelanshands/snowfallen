...
var max_degrees := 15.0
var recoil_constant := 4.0

func _process(_delta: float) -> void:
	if recoil_offset > 0.001:
		recoil_offset = lerp(recoil_offset, 0.0, 0.01) * (1.0 - (clamp(rotation_degrees.x + ((recoil_constant*2) if Input.is_action_pressed("crouch") else recoil_constant), 0.0, max_degrees) / max_degrees))
		rotation_degrees.x = rotation_degrees.x + recoil_offset
	rotation_degrees.x = clamp(rotation_degrees.x, 0.0, max_degrees)
